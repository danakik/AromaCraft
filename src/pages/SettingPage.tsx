    ...
    return (
        ...
        <div className="col flex flex-col items-center gap-3 p-2 -mt-3">
            <div className="block col-6">
                <ACKnob label="Темп. куба" color="purple" initialValue={Number(sortedData.tempCube)} readonly help={helpM.set_temp_cube_m} />
                <ACCounterLabel units="°C " value={Number(sortedData.tempCube)} label="Зміна темп. куба" help={helpM.set_temp_cube_m} />
            </div>
            <div className="block col-6">
                <ACKnob label="Темп. царги" color="orange" initialValue={Number(sortedData.tempCargi)} readonly help={helpM.set_temp_cargi_m} />
                <ACCounterLabel units="°C " value={Number(sortedData.tempCargi)} label="Зміна темп. царги" help={helpM.set_temp_cargi_m} />
            </div>
            </div>
            <div className="col flex flex-col items-center gap-3 p-2 -mt-3">
            <div className="block col-6">
                <ACKnob label="Темп. дефлегматора" color="red" initialValue={Number(sortedData.tempDef)} readonly help={helpM.set_temp_defl_m} />
                <ACCounterLabel units="°C " value={Number(sortedData.tempDef)} label="Зміна темп. дефл." help={helpM.set_temp_defl_m} />
            </div>
            <div className="block col-6">
                <ACKnob label="Темп. води" color="blue" initialValue={Number(sortedData.tempWoter)} readonly help={helpM.set_temp_water_m} />
    ...
    <div className="flex flex-column align-items-start justify-content-start gap-3 w-1/4 -ml-3">
        <div className="block p-4 flex-1 w-full">
            ...
            <div className="flex flex-row align-items-start justify-content-start w-full gap-2">
                <Button label="Зробити запит" style={{ backgroundColor: '#9e4ae7', borderColor: '#9e4ae7', color: '#fff' }} />
            </div>
            <div className="flex flex-row align-items-start justify-content-start w-full gap-2">
                <h3>Пристрій: </h3><h3>DESKTOP-5253</h3>
            </div>
        </div>
