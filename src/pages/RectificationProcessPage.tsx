...
const RectificationProcessPage = () => {
    ...
    <div className="flex flex-column w-1/4">
        <div className="grid grid-cols-2 w-full">
            <div className="col-6">
                <ACBlockTempSmall name="Куб" color="purple" temp={String(sortedData.tempCube)} help={helpM.temp_cube_m} />
            </div>
            <div className="col-6">
                <ACBlockTempSmall name="Царга" color="orange" temp={String(sortedData.tempCargi)} help={helpM.temp_cargi_m} />
            </div>
            <div className="col-6">
                <ACBlockTempSmall name="Дефлегматор" color="red" temp={String(sortedData.tempDef)} help={helpM.temp_defl_m} />
            </div>
            <div className="col-6">
                <ACBlockTempSmall name="Вода" color="blue" temp={String(sortedData.tempWoter)} help={helpM.temp_water_m} />
            </div>
        </div>
        <div className="grid grid-cols-2 gap-3 p-3">
            <div className="col flex flex-col align-items-center justify-content-center gap-3 p-2 -mt-3">
                <div className="block col-6">
                    <ACSlider label="Темп. відбору голів" color="blue" initialValue={Number(95.2)} help={helpM.temp_selection_heads_m} /><br/>
                    <ACCounterLabel units=' °C' value={Number(0.1)} label="Гістерезіс" help={helpM.gist_selection_heads_m} />
                </div>
                <div className="block col-6">
                    <ACSlider label="Темп. відбору тіла" color="orange" initialValue={Number(95.2)} help={helpM.temp_selection_body_m} /><br/>
                    <ACCounterLabel units=' °C' value={Number(0.1)} label="Гістерезіс" help={helpM.gist_selection_body_m} />
                </div>
            </div>
            <div className="col flex flex-col align-items-center justify-content-center gap-3 p-2 -mt-2" style={{ height: '200px' }}>
                <div className="block col-6" style={{ height: '200px' }}>
                    <ACSlider label="Темп. відбору хвостів" color="red" initialValue={Number(95.2)} help={helpM.temp_selection_tails_m} /><br />
                    <ACCounterLabel units='л/г' value={Number(0.38)} label="Швидкість" help={helpM.speed_selection_tails_m} />
                </div>
                <div className="block col-6" style={{ height: '200px' }}>
                    <ACSlider label="Зменш. відбору tКУБ" color="purple" initialValue={Number(95.2)} help={helpM.decrease_selection_m} /><br />
...
<div className="block p-3 w-full">
    ...
    <div className="block p-4 w-full">
        ...
        <div className="flex flex-row align-items-center justify-content-center w-full gap-2">
            <ACRegulator icon="ten" label="Потужн.розгону" value={Number(65)} units='%' help={helpM.power_acceleration_m} />
        </div>
        <div className="flex flex-row align-items-center justify-content-center w-full gap-2">
            <ACRegulator icon="ten" label="Потужн.відбору" value={Number(70)} units='%' help={helpM.power_selection_m}/>
        </div>
        <div className="flex flex-row align-items-center justify-content-center w-full gap-2">
            <ACRegulator icon="ten" label="Потужн.відбору(тіло)" value={Number(70)} units='%' help={helpM.power_selection_body_m}/>
        </div>
        <div className="flex flex-row align-items-center justify-content-center w-full gap-2">
            <ACRegulator icon="ten" label="Потужн.відбору(хвости)" value={Number(74)} units='%' help={helpM.power_selection_tails_m}/>
        </div>
    </div>
    ...
    <div className="block p-4 w-full">
        <h3>Швидкість</h3>
        <div className="flex flex-row align-items-center justify-content-center w-full gap-2">
            <ACRegulator icon="speed" label="Шв. відбору (голів)" value={Number(0.01)} units='л/г' help={helpM.speed_selection_heads_m} />
        </div>
        <div className="flex flex-row align-items-center justify-content-center w-full gap-2">
            <ACRegulator icon="speed" label="Шв. відбору (тіла)" value={Number(0.01)} units='л/г' help={helpM.speed_selection_body_m}/>
        </div>
        <div className="flex flex-row align-items-center justify-content-center w-full gap-2">
            <ACRegulator icon="speed" label="Шв. відбору (хвости)" value={Number(0.01)} units='л/г' help={helpM.speed_selection_tails_m}/>
        </div>
        <div className="flex flex-row align-items-center justify-content-center w-full gap-2">
    ...
    <div className="block p-4 w-full">
        ...
        <div className="flex flex-row align-items-center justify-content-center w-full gap-2">
            <ACRegulator icon="timer" label="Обмеження циклу" value={Number(1)} units='хв' help={helpM.border_cycles_m}/>
        </div>
        <div className="flex flex-row align-items-center justify-content-center w-full gap-2">
    ...
    <div className="block p-4 w-full">
        <h3>Інше</h3>
        <div className="flex flex-row align-items-center justify-content-center w-full gap-2">
            <ACRegulator icon="temp" color='white' label='Темп. переходу' value={Number(54)} units='°C' help={helpM.temp_transition_m}/>
        </div>
        <div className="flex flex-row align-items-center justify-content-center w-full gap-2">
            <ACRegulator icon="temp" color='white' label='Темп. відбору (хвостів)' value={Number(54)} units='°C' help={helpM.temp_selection_tails_m}/>
        </div>
        <div className="flex flex-row align-items-center justify-content-center w-full gap-2">
            <ACRegulator icon="temp" color='green' label='Темп. зупинки' value={Number(74)} units='°C' help={helpM.temp_stop_m}/>
        </div>
        <div className="flex flex-row align-items-center justify-content-center w-full gap-2">
            <ACRegulator icon="temp" color='red' label='Темп. аварії' value={Number(59)} units='°C' help={helpM.temp_breakdown_m} />
            </div>
        <div className="flex flex-row align-items-center justify-content-center w-full gap-2">
        ...
        <div className="flex flex-row align-items-center justify-content-center w-full gap-2">
            <ACRegulator icon="antena_bars" label='Стабілізація колони' value={Number(35)} units='хв' help={helpM.stabilisation_column_m} />
        </div>
    </div>
</div>
</div>
